...
export default {
  ...
  methods: {
    ...
    finishReports() {
      if (this.selection.length === 0) {
        this.$message.error('您尚未选择已处理的接报！')
        return
      }
      let ins = []
      for (let s of this.selection) {
    ...
    getTemplateList() {
      this.tableData = difficulties
    },
    editReport() {
      let id = queryDifficultyById(this.form.id)
      difficulties[id].name = this.form.name
      difficulties[id].code = this.form.code
      difficulties[id].type = this.form.type
      difficulties[id].reporter = this.form.reporter
      difficulties[id].phone = this.form.phone
      difficulties[id].company = this.form.company
      difficulties[id].lastModifier = this.form.lastModifier
      difficulties[id].lastModifiedTime = dateToString(new Date())
      difficulties[id].status = this.form.status
    ...
    search() {
      if (this.keyword === null || this.keyword.length === 0) {
        ...
        this.tableData =queryDifficultiesByBlur(this.keyword)
      }
    },
    goSearch() {
      let that = this
